...
let rec translate_build_command pkg_name arch = function
    | List (pos, (String (p1, "jbuilder"))::(String (_, "build"))::next) ->
        let command = [ "jbuilder"; "build"; "-x"; arch; "-p"; pkg_name ] in
        let new_cmd = List.map (to_cmd p1) command in
        List (pos, new_cmd), Jbuilder
    | List (pos, values) -> 
        let res = List.map (translate_build_command pkg_name arch) values in
...
let rec translate_items pkg_name arch deps new_ast action = function
    | Variable (pos, "depends", List (p2, values))::next -> 
        ...
        translate_items pkg_name arch deps new_ast action next 
    | Variable (pos, "build", values)::next ->
        let values, action = translate_build_command pkg_name arch values in
        let new_ast = Variable (pos, "build", values)::new_ast in 
        translate_items pkg_name arch deps new_ast action next 
    | s::next -> translate_items pkg_name arch deps (s::new_ast) action next 
...
let translate (package_definition : opamfile) pkg_name arch = 
    let new_ast, deps, action = translate_items pkg_name arch [] [] None package_definition.file_contents in
    {
